...
import type { OllamaFetchProgress } from './ollama-download';
...
contextBridge.exposeInMainWorld('electron', {
  ...
  ollama: {
    ...
    /** Subscribe to one-time runtime-download progress emitted while
     *  startBundled() fetches the engine. Returns an unsubscribe function. */
    onFetchProgress: (cb: (p: OllamaFetchProgress) => void): (() => void) => {
      const listener = (_e: unknown, p: OllamaFetchProgress): void => cb(p);
      ipcRenderer.on('ollama:fetch-progress', listener);
      return () => ipcRenderer.removeListener('ollama:fetch-progress', listener);
    },
  },
